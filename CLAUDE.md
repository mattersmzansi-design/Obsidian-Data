...
3. **`<script>`** (near the bottom, ~line 683) — all interactivity as plain
   functions on the global scope, wired up via inline `onclick=` attributes.
...
- Commit with clear, descriptive messages and push with
  `git push -u origin claude/claude-md-docs-9gk1dt`.
